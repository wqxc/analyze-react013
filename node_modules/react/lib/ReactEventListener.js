...
function handleTopLevelWithoutPath(bookKeeping) {
  ...
  // getFirstReactDOM得到React根组件的最外层包括的DOM元素,也就是document。
  var topLevelTarget = ReactMount.getFirstReactDOM(getEventTarget(bookKeeping.nativeEvent)) || window;
  // topLevelTarget是触发事件的当前元素，这里的方法就是为了找到最近的一个DOM元素，并由这个DOM元素来
  // 向上查找，其全部父元素。就是找到事件触发的时候冒泡全过程（经过的所有元素。）
  // 方法名就是 withoutPath，在没有path属性的情况下来找到冒泡全过程。下边有方法是根据有path属性的时候找到冒泡经过的全部元素。
